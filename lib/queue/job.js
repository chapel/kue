  ...
  return function(err, ids){
    ...
    ids.forEach(function(id){
      ...
      exports.get(id, function(err, job){
        ...
        if (!job) return --pending, exports.purgeJob(id)
        jobs[job.id] = job;
...
exports.purgeJob = function(id){
  var client = this.client

  client.zrem('q:jobs', id);

  client.keys('q:jobs:*', function(err, keys){
    if (err) return;
  ...
  if (exports.search) getSearch().remove(id);
  client.del('q:job:' + id, noop);
  client.del('q:job:' + id + ':log');
};
...
Job.prototype.remove = function(fn){
  this.removeState();
  if (this.search) getSearch().remove(this.id);
...
Job.prototype.update = function(fn){
  ...
  // search data
  if (this.search) getSearch().index(json, this.id);
};
